...
def get_model(x0, size, prior_cov=1.0, obs_cov=0.1, shift=0.0, obs_gap=0.2):
    ...
    def lorenz_96(x0, obs_gap_=0.2):
        return scipy.integrate.solve_ivp(lorenz96_f, [0.0, obs_gap_], x0, method='RK45', t_eval=[obs_gap_]).y.T[0]
    ...
    def gen_path(length):
        path = np.zeros((length, dim), dtype = 'float64')
        x = x0
