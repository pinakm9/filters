	...
	def plot_weighted_ensembles_2D(self, ensembles, labels = None, weights = None, colors = None, file_path = 'ensemble_plot.png.',\
		...
		for k, ensemble in enumerate(ensembles):
			...
			for i, pt in enumerate(ensemble):
				...
				ax.scatter(pt[0], pt[1], s = sz, color = colors[k], label = labels[k] if i == 0 else None, alpha = alpha)
...
def plot_ensemble_evol(db_path, hidden_path, time_factor=1,\
	...
	#"""
	w = np.ones(len(getattr(hdf5.root.prior_ensemble, 'time_' + str(0)).read().tolist()))
	observations = hdf5.root.observation.read().tolist()
	for t, observation in enumerate(observations):
		ens_pr = getattr(hdf5.root.prior_ensemble, 'time_' + str(t*time_factor)).read().tolist()
		ens_po = getattr(hdf5.root.posterior_ensemble, 'time_' + str(t*time_factor)).read().tolist()
		...
		ep.plot_weighted_ensembles_2D(ensembles = [ens_pr, ens_po], weights = [w, w], labels = ['prior', 'posterior'],\
	...
	#"""
	ep.stich(folder = folder, img_prefix = 'ensembles', pdf_name = 'enkf_evolution.pdf', clean_up = True,\
